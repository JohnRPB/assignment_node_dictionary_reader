...
let onData2 = (data, dictionaryData) => {
  let search = require('./search.js');
  
          num = data.trim();
          console.log(dictionaryData);
          
          process.stdin.removeListener('data', onData2);
          if (num === "1") {
           ...
           process.stdin.on("data", (data) => {
             ...
             console.log("displaying exact match:");
             console.log(search.exactMatch(dictionaryData, data));
             process.exit();
           });
          } else if (num === "2") {
            console.log(search.partialMatch(dictionaryData, 'sci'));
          } else if (num === "3") {
            console.log(search.beginMatch(dictionaryData, 'ab'));
          } else if (num === "4") {
            console.log(search.endsMatch(dictionaryData, 'id'));
          } else {
  ...
  var onData = (data) => {
    ...
    if (data === 'next') {
      ...
      loader.loadFile(pathname).then((result) => {
        ...
        console.log("Enter the number for search"); 
        ...
        process.stdin.setEncoding('utf8');
        process.stdin.on("data", onData2, data, dictionaryData);
